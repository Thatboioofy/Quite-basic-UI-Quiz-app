...
def accent_colour(new_col):
    ...
    checkbox1.configure(fg_color=acccolour, hover_color=highlight)
    checkbox2.configure(fg_color=acccolour, hover_color=highlight)
    checkbox3.configure(fg_color=acccolour, hover_color=highlight)
    checkbox4.configure(fg_color=acccolour, hover_color=highlight)
...
frame.grid(row=0, padx=100, pady=(60,0))
...
def CheckMC():
  ...
  MC3 = checkbox3.get()
  MC4 = checkbox4.get()
  MC_Correct = Conf[Q]['correct-ans'].split(',')

  #Check for Box 1
  if MC_Correct.count('Op1') == 1 and MC1 == 'on':
  ...
  #Check for Box 2
  if MC_Correct.count('Op2') == 1 and MC2 == 'on':
   print("MC2 Correct")
  ...
  #Check for Box 3
  if MC_Correct.count('Op3') == 1 and MC3 == 'on':
   print("MC3 Correct")
  ...
  #Check for Box 4
  if MC_Correct.count('Op4') == 1 and MC4 == 'on':
   print("MC4 Correct")
   ...
   #Overall Checker 
  Amount = len(MC_Correct)
  if Amount ==MC_Corr and MC_Inc == True:
    print("Incorrect")
    incc = incc +1
    app.after(1000, selection)
  elif Amount == MC_Corr:
    print("Overall correct")
    cor = cor+1
    app.after(1000, selection)
  else:
    print("Incorrect")
...
def MC():
  ...
  global btn4
  global Submit
  Submit = customtkinter.CTkButton(app, text="Submit", width=60, command=CheckMC,   fg_color=acccolour, hover_color=highlight)
  Submit.grid(row=0, column=0, padx=15, pady=(60,0), sticky="se")
  if butn_loaded == 1:
   btn1.grid_forget()
   btn2.grid_forget()
   btn3.grid_forget()
   btn4.grid_forget()
  checkbox1 = customtkinter.CTkCheckBox(frame, text="", onvalue="on", offvalue="off", width=50)
  checkbox1.grid(row=1, column=0, padx=10, pady=(10, 5), sticky="new")
  checkbox2 = customtkinter.CTkCheckBox(frame, text="",onvalue="on", offvalue="off", width=50)
  checkbox2.grid(row=2, column=0, padx=10, pady=5, sticky="new")
  checkbox3 = customtkinter.CTkCheckBox(frame, text="",onvalue="on", offvalue="off", width=50)
  checkbox3.grid(row=3, column=0, padx=10, pady=5, sticky="new")
  checkbox4 = customtkinter.CTkCheckBox(frame, text="",onvalue="on", offvalue="off", width=50)
  checkbox4.grid(row=4, column=0, padx=10, pady=(5,10), sticky="new")
    ...
    if Comp != Max:
        ...
        if Conf.get(Q, 'MC-mode') == '1':
           ...
           if butn_loaded == 1:
            ...
            btn3.grid_forget()
            btn4.grid_forget()
        if Conf.get(Q, 'MC-mode') == '0' and MCrun == 0:
           default()
        TmpQ = Q
        if Conf.has_option(Q, 'Correct-ans') == True:
